...
namespace Be.Windows.Forms
{
    ...
    public class FileByteProvider : IByteProvider, IDisposable
    {
        ...
        public bool HasChanges() => _writes.Count > 0;
        ...
        {
            if (this._readOnly) throw new Exception("File is in read-only mode.");
        ...
        /// </summary>
        public void RejectChanges() => _writes.Clear();
        ...
        public byte[] ReadBytes(long index, int length)
        {
            ...
            {
                length = (int)(Length - 1 - index);
                Array.Resize(ref buffer, length);
            }
            _fileStream.Read(buffer, (int)index, length);
            for (int idx = 0; idx < buffer.Length; idx++) if (_writes.Contains(idx)) buffer[idx] = _writes[idx];
            return buffer;
        }
        ...
        {
            if(_writes.Contains(index)) _writes[index] = value;
            else _writes.Add(index, value);
